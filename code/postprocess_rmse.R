...
# Should be one of sum,high, low, and piecewise, tree, and lss
dgp = "tree"
...
for (dgp in c("sum","high", "low", "piecewise", "tree", "lss")) {
  ...
  for (file in dir("results/coverage2/")) {
    if (grepl(pattern = dgp, x = file)) {
      #print(file)
      f <- readRDS(file = paste0("results/coverage2/",file))
  ...
  results[-1,] %>%
    filter(n>100) %>%
    group_by(n) %>%
    mutate(rmse_one_chain = mean(rmse[nchain==1])) %>%
    group_by(n, nchain, exp) %>%
    summarise(mean_rmse = mean(rmse),
              mean_coverage = mean(cov),
              sd_coverage = sd(cov),
              mean_rmse = mean(rmse)/rmse_one_chain,
              sd_rmse = sd(rmse)/rmse_one_chain) %>%
    group_by(n) %>%
    mutate(mean_rmse = mean_rmse / mean_rmse[nchain==1],
           sd_rmse = sd_rmse / mean_rmse[nchain==1]) %>%
  ...
  ggsave(plot_i,filename = paste0("results/figures/coverage2/",dgp,"rmse.pdf"), height = 4, width = 6)
...
ggsave(p_final,filename = paste0("results/figures/coverage2/all_rmse.pdf"), height = 11, width = 8)
